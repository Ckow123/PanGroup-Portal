    ...
    >
      ...
      <div aria-hidden="true" className="hero-atmosphere absolute inset-0 pointer-events-none">
        ...
        <div className="hero-particles absolute inset-0">
          {Array.from({ length: 7 }, (_, index) => <span key={index} className="hero-particle" />)}
        </div>
      </div>
      ...
      <div className="hero-layout relative z-10 mx-auto flex min-h-[max(42rem,100svh)] w-full max-w-7xl items-start px-4 pb-14 pt-28 sm:px-8 sm:pb-20 sm:pt-[clamp(9rem,18vh,11rem)] lg:px-10 xl:px-8">
        <div className="hero-copy w-full max-w-[36rem] text-white">
          <p className="hero-reveal hero-welcome mb-5 flex items-center gap-3.5 text-xs font-semibold uppercase tracking-[0.22em] text-white/84 sm:text-sm">
          ...
          >
            <span className="hero-reveal hero-name block">PAN GROUP</span>
            <span className="hero-reveal hero-accent mt-3 block font-serif text-[0.45em] font-normal italic leading-[1.18] tracking-[-0.012em] text-white/94 sm:mt-4">
              Breast Cancer Research
            </span>
          </h1>

          <div className="hero-reveal hero-details mt-7 max-w-[32rem] sm:mt-8">
