...
export default function ResourcesSection() {
  return (
    <section
      ...
      className="resources-world relative overflow-hidden border-t border-white/60 bg-blush px-4 py-20 text-ink sm:px-6 sm:py-28 lg:px-8 lg:py-32"
    >
      <div className="relative mx-auto max-w-6xl">
        <header className="grid min-w-0 items-end gap-7 lg:grid-cols-[1fr_24rem] lg:gap-20">
          <div className="min-w-0">
            ...
            <h2
              id="resources-title"
              className="section-title min-w-0 font-display text-[clamp(2.35rem,5vw,4.6rem)] font-semibold leading-[1.05] tracking-[-0.035em]"
            >
              Four ways into
              <span className="block font-serif font-normal italic text-lilac-ink">the work.</span>
            </h2>
          </div>
          <p className="section-copy max-w-md text-base leading-7 text-ink/70 sm:text-lg sm:leading-8">
        ...
        <div className="mt-12 grid gap-4 sm:mt-16 md:grid-cols-2 md:gap-5">
          {resources.map((resource) => (
            <article
              key={resource.id}
              id={resource.id}
              className="resource-card rounded-[1.5rem] border p-6 backdrop-blur-sm sm:p-7"
            >
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.145em] text-ink/70">
                  <span aria-hidden="true" className={`size-2.5 rounded-full ${resource.accent}`} />
                  <span>{resource.cue}</span>
                </div>
                <span className="rounded-full border border-lilac/25 bg-white/62 px-3 py-1.5 text-sm text-ink/72" lang="zh-Hans">
                  {resource.label}
                </span>
              </div>

              <div className="mt-9 sm:mt-10">
                <p className="mb-3 text-xs font-medium tracking-[0.12em] text-ink/70">
                  {resource.index}
                </p>
                <h3 className="font-display text-2xl font-semibold tracking-[-0.03em] sm:text-3xl">
                  {resource.title}
                </h3>
                <p className="section-copy mt-3 max-w-lg text-base leading-7 text-ink/70">
